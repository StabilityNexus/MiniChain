...
def test_wrong_chain_id_rejected(alice, bob, funded_state):
    ...
    assert not funded_state.apply_transaction(
        tx
    ), "Transaction for a different chain_id must be rejected."
    # Ensure the rejected transaction did not mutate the ledger
    assert (
        funded_state.get_account(alice_pk)["balance"] == 100
    ), "Alice's balance must remain unchanged after a cross-chain rejection."
    assert (
        funded_state.get_account(alice_pk)["nonce"] == 0
...
def test_replay_attack_same_nonce_rejected(alice, bob, funded_state):
    ...
    assert (
        funded_state.get_account(alice_pk)["balance"] == 90
    ), "Alice's balance must not change after a rejected replay."
    assert (
        funded_state.get_account(alice_pk)["nonce"] == 1
...
def test_sequential_nonces_accepted(alice, bob, funded_state):
    ...
    assert (
        funded_state.get_account(alice_pk)["nonce"] == 2
    ), "Alice's nonce should advance to 2 after two accepted transactions."
    assert (
        funded_state.get_account(alice_pk)["balance"] == 80
    ), "Alice's balance should be 80 after two 10-coin transfers."
    assert (
        funded_state.get_account(bob_pk)["balance"] == 20
