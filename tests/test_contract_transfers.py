...
"""
        deploy_tx = self._sign(
            Transaction(self.sender_pk, None, amount=100, nonce=0, data=code, fee=1000)
        )
...
"""
        ...
        self.assertEqual(
            receipt2.error_message, "Insufficient contract balance for transfers"
        )
        ...
        self.assertEqual(
            self.state.get_account(self.sender_pk)["balance"], 8900 - receipt2.gas_used
        )
...
"""
        deploy_tx = self._sign(
            Transaction(self.sender_pk, None, amount=0, nonce=0, data=code, fee=1000)
        )
        receipt = self.state.apply_transaction(deploy_tx)
        self.assertEqual(receipt.status, 1)
        contract_addr = receipt.contract_address
